...
# %%
class bins_class:
    def __init__(self, y, n_bins = 5):
        self.n_bins = n_bins
        min_val = np.min(y)
        max_val = np.max(y)
        self.bins_cutoff = np.linspace(min_val, max_val, n_bins + 1)[1:]

        bins = [[] for _ in range(self.n_bins)]
        for i in range(len(y)):
            bin_index = sum(y[i] > self.bins_cutoff)
            bins[bin_index].append(i)

        bin_lengths = [len(b) for b in bins]
        sorted_indices = np.argsort(bin_lengths)

        self.bin_ranks = [0] * self.n_bins
        for rank, idx in enumerate(sorted_indices):
            self.bin_ranks[idx] = rank + 1

    def set_idx_in_bins(self, y_test):
        bins = [[] for _ in range(self.n_bins)]
        for i in range(len(y_test)):
            bin_index = sum(y_test[i] > self.bins_cutoff)
            bins[bin_index].append(i)
...
def run_benchmark(dataset_names: list[str], directory_result: str, verbose: bool = False, resume: bool = False):
        ...
        try:
            ...
            for fold_number, (train_index, test_index) in enumerate(cross_validation.split(dataset.X, dataset.y)):
                ...
                try:
                    ...
                    idx_y_test_in_bins = bins.set_idx_in_bins(test_data.y)
                ...
                for y_pred, method in zip([y_pred_pilot, y_pred_pilotW, y_pred_cartW], ["Pilot", "PilotW", "CartW"]):
                    ...
                    for bin_rank, idx_y_test in zip(bins.bin_ranks, idx_y_test_in_bins):
                        if idx_y_test != []:
                            mse = float(mean_squared_error(test_data.y[idx_y_test], y_pred[idx_y_test]))
                            if len(idx_y_test) > 2:
                                r2 = float(r2_score(test_data.y[idx_y_test], y_pred[idx_y_test]))
                                nmse = float(mean_squared_error(test_data.y[idx_y_test], y_pred[idx_y_test]) / np.var(
                                    test_data.y[idx_y_test], ddof=1))
                            else:
                                nmse = None; r2 = None
                        else:
                            mse = None; nmse = None; r2 = None
    ...
    df_fold_avg = (
        df.groupby(['Dataset', 'Method', 'bin'], as_index=False)
        .agg({
